...
export default function Crew() {
  ...
  const handleSearch = useCallback(() => {
    ...
    crews.map((crew) => {
      ...
      if (
        crew.name.toLocaleLowerCase().includes(lowered) ||
        crew.skills.some((skill) =>
          skill.toLocaleLowerCase().includes(lowered)
        ) ||
        crew.location.includes(lowered) ||
        crew.crew?.toLocaleLowerCase().includes(lowered)
      ) {
        found.push(crew);
      }
    });
    ...
    <div className="min-h-screen">
      ...
      >
        <div className="bg-white rounded-2xl p-5">
          ...
          <button className="submit mt-4">크루 찾기</button>
        </div>
      </form>
      <div>
        <div className="flex-row justify-between p-2">
          <p className="font-semibold">총 {items.length}개의 크루</p>
