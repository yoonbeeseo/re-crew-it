...
export default function RForm({ closeFn, user }: Props) {
  const { resume, onChangeR, payload, onChangeResumes, resumes } = use(
  ...
  const handleSubmit = useCallback(async () => {
    ...
    const updateFn = async () => {
      const userData = await fetch(`/api/users?id=${user?.id}`);
      const users = (await userData.json()) as User[];
      await fetch(`api/users/${user?.id}`, {
        method: "PATCH",
        body: JSON.stringify({ ...users[0], updated_at: new Date().getTime() }),
      });
    };
    if (payload) {
      //Todo: 수정 로직 ㄱ
