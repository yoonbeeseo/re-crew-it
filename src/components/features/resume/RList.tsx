...
export default function RList({ showForm }: Props) {
  ...
  const ResumeItem = useCallback(
    (resume: Resume) => {
      ...
      return (
        <div className="p-2 rounded border-gray-200 hover:bg-gray-50">
          <div>
            <p className="font-semibold">
              {resume.company}{" "}
              <span className="font-normal px-1 rounded bg-theme text-white">
                {resume.job}
              </span>
            </p>
            <p>
              {new Date(resume.startDate).toLocaleDateString()}~
              {resume.endDate === "현재까지"
                ? resume.endDate
                : new Date(resume.endDate).toLocaleDateString()}
            </p>
          </div>
